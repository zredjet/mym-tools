...
## v0.1.0-alpha.15 の主な変更

- ダイアグラムで、draw.ioの「ファイル → 形式を指定してエクスポート」からPNG / SVG / XMLを書き出せるように修正（倍率・余白・透過などをdraw.io側で指定可能）。JPEG / PDF / HTMLなど他の形式は理由を表示して書き出さない
- ダイアグラム上部の「PNG」書出しを2倍の解像度に変更
