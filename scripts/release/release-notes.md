...
## v0.1.0-alpha.17 の主な変更

- BinaryFormatter解析（NRBF）で、値の大きいファイル（例: 0.2 MB・400ノード程度）を読み込むと、ノード数とルート型だけが表示されてツリーもエラーも出ないことがある不具合を修正
- 500ノードを超えるファイルで、ツリーの末尾が欠けたまま表示される可能性があった問題を修正。受信したノード数が解析結果と一致しない場合は、ツリーを表示せずエラーを表示する
