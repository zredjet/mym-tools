...
## v0.1.0-alpha.9 の主な変更

- Mermaidの現在の正常なプレビューをSVGまたは白背景・2倍解像度のPNGとして保存可能
- Mermaid画像書出しに20MiB、最大16,384px、16,777,216画素の上限と能動コンテンツ検証を追加
- draw.ioエディタを同梱済み日本語資産で日本語固定し、追加ダウンロードなしで利用可能
- draw.ioのPNG書出しを現在表示しているページへ変更し、複数ページ図を直感的に保存可能
- SVG／PNGの検証と原子的書込みを共通化し、画像生成中の重複書出しを防止
