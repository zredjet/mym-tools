...
#### 7.15.5 paths-ignore で skip された required check は GitHub 上で BLOCKED 扱い (§2.2 の前提崩れ)

- **発覚**: Day 2 で branch protection に required check 6 個を登録した直後、smoke test PR (`README.md` のみの 1 行変更) で `mergeStateStatus: BLOCKED / statusCheckRollup: []` 発生
- **真因**: GitHub の実挙動では、`paths-ignore` で workflow 自体が trigger されない場合、required check は「Expected」状態で**永遠に未報告 → BLOCKED**。本 ADR §2.2 / Round 2 M-4 で前提とした「skip = success として扱われる」は **誤り** (skip = success が成立するのは、workflow が trigger された上で `if:` 条件で job が skip された場合のみ)
- **対策の選択肢**:
  - 案 A: `ci-noop.yml` を別ファイルで用意し、`paths` で `docs/**` 等にマッチさせて同名 check 6 個を success 報告 → 複合 PR (docs と code 両方変更) で **同名 check が 2 回報告される race condition** リスク
  - **案 B (採用)**: `paths-ignore` を撤廃して全 PR で 6 ジョブ走らせる。public repo 化済 (§1.2 / §2.8) で Free tier 無制限のため、当初試算 (PR 1 回 ≒ 100 分) の課金懸念は実質ゼロ
- **対応**: `.github/workflows/ci.yml` から `paths-ignore` を削除。本 ADR §2.2 / §2.10 / §4.3 / §5 の `paths-ignore` 記述は **§7.15.5 への参照を併記して効力を弱める** (本 ADR は決定変更ではなく追記専用ポリシーのため、原文は残す)
- **将来再評価**: private 化が必要になったタイミングで案 A の noop workflow パターン or `if:` ベース skip パターンを再検討する
