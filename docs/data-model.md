...
- **検索語の長さで経路を切り替える** (StorageService 内部の検索 API で実装):
  - 入力は前後空白を除き、空白で語に分割する。語同士は AND (全語を含む item を返す)
  - 全語 ≧ 3 文字 → `items_fts MATCH ?` で検索。各語は FTS5 フレーズ (`"語"`、語中の `"` は `""`) として渡し、利用者の入力を FTS5 のクエリ構文 (`-` / `:` / `*` / `NEAR` / 括弧など) として解釈させない。`#ff0000` や `https://…` も部分文字列として一致する
  - 3 文字未満の語を 1 つでも含む → `items` テーブル直接の `LIKE '%語%'` フォールバック (対象カラム: `title` / `tags` / `search_text`、語ごとに OR、語同士は AND)
