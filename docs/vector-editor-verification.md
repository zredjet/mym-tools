...
最終確認: 2026-09-12。仕様は[ADR-0021](decisions/0021-offline-vector-editor.md)。実装とローカル検証は完了。以下の未実施項目を含むため、両OSの配布受入完了とは扱わない。
...
エディタの資産は345ファイル、約3.6MB。標準Editor.jsは変更せず、アダプター、入力ポリシー、日本語補完を同梱する。source mapは監査時にだけ参照する。
...
初回実アプリ検証で不足していた新規IPCのACL、およびSDKの`onCloseRequested`が使用するclose/destroy権限を修正し、終了確定まで再検証した。権限は親ウィンドウのlocal capabilityだけに付与し、iframe originへは付与しない。macOSメニューのCmd+Qもwindow close経路を使う。最終レビューで追加した外部cursor/color-profile拒否、文書ID付き操作要求、画像デコード後の容量再判定は、その後に自動テストとChromiumで再検証した。これらの差分についてネイティブ受入一式は繰り返していない。
...
| 対象 | 結果 |
|---|---|
| macOS arm64、通常identifier `com.zredjet.mymtools` | **54,755,524 bytes**、上限80,000,000 bytesまで25,244,476 bytes |
| ZIP CRC・内部パス | 成功。MyMyTools.app、実行ファイル、NRBF sidecarを確認 |
| SHA-256 | `3fe1b4c155bdc806e5e7534ec23ccf32a26395f261aaa914f57cfbbdcfe2c39d` |
