//! SVG-Editクライアント資産を、IPC権限を持たないloopback originから配信する。
...
pub async fn vector_editor_url(app: AppHandle) -> Result<String, AppError> {
    let mut server_port = SERVER_PORT
        .lock()
        .map_err(|_| AppError::Io("vector asset server lock is poisoned".into()))?;
    if let Some(port) = *server_port {
        return Ok(editor_url(port));
    }

    let std_listener = StdTcpListener::bind((std::net::Ipv4Addr::LOCALHOST, 0))
        .map_err(|error| AppError::Io(format!("failed to bind vector asset server: {error}")))?;
    std_listener.set_nonblocking(true).map_err(|error| {
        AppError::Io(format!("failed to configure vector asset server: {error}"))
    })?;
    let port = std_listener
        .local_addr()
        .map_err(|error| AppError::Io(format!("failed to inspect vector asset server: {error}")))?
        .port();
    let listener = TcpListener::from_std(std_listener)
        .map_err(|error| AppError::Io(format!("failed to start vector asset server: {error}")))?;
...
async fn serve(listener: TcpListener, app: AppHandle, port: u16) {
    ...
    loop {
        ...
        match listener.accept().await {
            ...
            Ok(_) => {}
            Err(error) => {
                tracing::error!(%error, "vector asset server stopped");
                break;
            }
        }
    }
