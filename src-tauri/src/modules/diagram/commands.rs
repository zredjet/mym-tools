...
// File I/O and XML / image validation run away from the WebView (main) thread.
#[tauri::command]
pub async fn diagram_read_file(path: String) -> Result<String, AppError> {
    tauri::async_runtime::spawn_blocking(move || read_file(path)).await?
}

#[tauri::command]
pub async fn diagram_write_file(
    path: String,
    format: String,
    data: String,
) -> Result<(), AppError> {
...
fn read_file(path: String) -> Result<String, AppError> {
    ...
    // metadata 確認後にファイルが伸びても上限 + 1 byte までしか読まない
    let mut xml = String::new();
    fs::File::open(&path)
        .and_then(|file| {
            file.take(MAX_DIAGRAM_BYTES as u64 + 1)
...
mod tests {
    ...
    fn reads_valid_xml_and_rejects_other_extensions() {
        ...
        assert_eq!(read_file(path.display().to_string()).unwrap(), XML);

        let invalid = directory.path().join("sample.txt");
        fs::write(&invalid, XML).unwrap();
        assert!(read_file(invalid.display().to_string()).is_err());
    }
    ...
    fn rejects_invalid_and_oversized_imports() {
        ...
        assert!(read_file(oversized.display().to_string()).is_err());
    }
    ...
    fn writes_each_supported_format_and_replaces_existing_file() {
        ...
        write_file(drawio.display().to_string(), "drawio".into(), XML.into()).unwrap();
        assert_eq!(fs::read_to_string(drawio).unwrap(), XML);

        let svg = directory.path().join("sample.svg");
        write_file(svg.display().to_string(), "svg".into(), "<svg/>".into()).unwrap();
